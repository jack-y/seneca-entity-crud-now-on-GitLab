...
describe('then thenForEntitySync', function () {
  after((done) => {
  ...
  it('no entity', function (fin) {
    /* Fires the test */
    processThen.thenForEntitySync(null, null, getThenArgsDefault())
    .then(function (result) {
      /* Checks the result */
      expect(result.length).to.equal(0)
      fin()
  ...
  it('no then arguments', function (fin) {
    /* Fires the test */
    processThen.thenForEntitySync(null, getEntity(), null)
    .then(function (result) {
      /* Checks the result */
      expect(result.length).to.equal(0)
      fin()
  ...
  it('no then action', function (fin) {
    /* Fires the test */
    processThen.thenForEntitySync(null, getEntity(), {actions: []})
    .then(function (result) {
      /* Checks the result */
      expect(result.length).to.equal(0)
      fin()
  ...
  it('then action on error', function (fin) {
    ...
    /* Fires the test */
    processThen.thenForEntitySync(null, getEntity(), getThenArgsDefault())
    .catch(function (err) {
      /* Checks the result */
  ...
  it('then action OK', function (fin) {
    ...
    .then(function (result) {
      ...
      expect(result[expected - 1].entity.id).to.equal('i1')
