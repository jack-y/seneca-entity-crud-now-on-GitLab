...
describe('then thenForListAsync', function () {
  after((done) => {
  ...
  it('empty list', function (fin) {
    /* Fires the test */
    processThen.thenForListAsync(null, [], getThenArgsDefault())
    .then(function (result) {
      /* Checks the result */
      expect(result.length).to.equal(0)
      fin()
  ...
  it('no then arguments', function (fin) {
    /* Fires the test */
    processThen.thenForListAsync(null, getListResult(), null)
    .then(function (result) {
      /* Checks the result */
      expect(result.length).to.equal(0)
      fin()
  ...
  it('no then action', function (fin) {
    /* Fires the test */
    processThen.thenForListAsync(null, getListResult(), {actions: []})
    .then(function (result) {
      /* Checks the result */
      expect(result.length).to.equal(0)
      fin()
  ...
  it('then action on error', function (fin) {
    ...
    /* Fires the test */
    processThen.thenForListAsync(null, getListResult(), getThenArgsDefault())
    .catch(function (err) {
      /* Checks the result */
